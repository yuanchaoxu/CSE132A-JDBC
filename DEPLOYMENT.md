...
### If you deploy with Wrangler CLI (fix for "Missing entry-point")

If you run `wrangler deploy` without a Worker script, Wrangler throws:
`Missing entry-point to Worker script or to assets directory`.

This repository now includes `wrangler.jsonc` with an `assets.directory` setting,
so static files can be deployed directly.
...
Or explicitly deploy Cloudflare Pages from the current folder:
